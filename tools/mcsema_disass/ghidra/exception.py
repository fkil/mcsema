...
def format_lsda_actions(action_tbl, act_ea, type_addr, type_enc, act_id):
  ...
  while True:
    ...
    if ar_filter > 0:
      ...
      DEBUG("catch type typeinfo = {:x} {} {}".format(type_ea, getSymbolName(type_ea), ar_filter))
  ...
  for entry in heads:
    ...
    if index < len(heads) - 1:
      ...
      while heads[index] <= ea < heads[index + 1]:
        inst, byts = decode_instruction(ea)
        if not inst:
          break
        ilen = len(byts)
        block = EHBlocks(ea, ea + ilen)
        ea = ea + ilen
        block_set.add(block)
    index = index + 1
...
def format_lsda(lsda_ptr, start_ea, range = None,  sjlj = False):
  ...
  while ea < action_tbl:
    if sjlj:
      ...
      cs_start += lpstart
      DEBUG("ea {:x}: cs_start[{}] = {:x}  ({})".format(ea, i, cs_start, getSymbolName(start_ea)))
      ea = next_ea
      heads.add(cs_start)
...
def recover_frame_entries(segment):
  DEBUG("Recover entries from section : {}".format(block.getName()))
  ea = segment.getStart()
  end_ea = segment.getEnd()
...
def recover_exception_table():
  ...
  """
  ehFrameSeg = getSegmentByNames([".eh_frame", "__eh_frame"])
  recover_frame_entries(ehFrameSeg)
