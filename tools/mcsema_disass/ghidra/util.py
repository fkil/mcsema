...
_DEBUG_FILE = None
_DEBUG_PREFIX = ""
_INFO = { "procName" : "foo", "filetype": "foo" }
...
def decode_instruction(ea):
  ...
  inst = currentProgram.getListing().getCodeUnitContaining(ea)
  if not inst: 
      return _BAD_INSTRUCTION
  return inst, inst.getBytes()
...
_IGNORE_DREF = (lambda x: [BADADDR])
_IGNORE_CREF = (lambda x, y: [BADADDR])
...
def is_thunk(ea):
  ...
  flags = idc.get_func_attr(ea, idc.FUNCATTR_FLAGS)
  return (BADADDR != flags) and 0 < flags and 0 != (flags & 0x00000080)
...
def make_head(ea):
  ...
  if not idc.is_head(flags):
    ...
    idc.create_data(ea, idc.FF_BYTE, 1, BADADDR)
    idaapi.auto_wait()
    return is_head(ea)
  return True
